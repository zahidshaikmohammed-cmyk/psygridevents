...
def _similarity(
    a: RawObservation,
    b: RawObservation,
    token_cache: dict[int, set[str]] | None = None,
) -> tuple[float, str]:
    ...
    if token_cache is None:
        ...
        left, right = token_cache[id(a)], token_cache[id(b)]
...
def cluster_stories(observations: list[RawObservation], threshold: float = 0.42) -> list[StoryCluster]:
    ...
    """
    token_cache = {id(observation): _tokens(observation) for observation in observations}
    clusters: list[list[RawObservation]] = []
    for observation in observations:
        best_index = None
        best_score = 0.0
        for index, cluster in enumerate(clusters):
            score = max(_similarity(observation, member, token_cache)[0] for member in cluster)
            if score > best_score:
